def validate_parameters(engagement_score_param, cut_off_score_param)
    ...
    if engagement_score_param !~ /^\d+$/
        content_type :json
        status 400
        return { :error => "Invalid parameter. The calculated engagement score for your attendance was not an integer." }.to_json
    end
    ...
    if engagement_score < 0 || engagement_score > 100
        content_type :json
        status 400
        return { :error => "Invalid parameter. The calculated engagement score for your attendance must be between 0 and 100." }.to_json
    end
